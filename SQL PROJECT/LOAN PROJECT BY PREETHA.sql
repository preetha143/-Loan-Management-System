...
alter table customer_income modify `customer id` varchar(50);
alter table customer_income modify loan_id varchar(50);
alter table customer_income add primary key (`customer id`);
describe customer_income;

select * from customer_income;
select*from customer_income_grade;

create table customer_income_grade select *,
case
...
create table customer_info select *,
case
...
from customer_income_grade;


create table loan_status (loan_id varchar(30) primary key,customer_id varchar(55),loan_amount varchar(50),loan_amount_term int,cibil_score int);
select *from loan_status;

delimiter //
create trigger loan_amount_verify before insert on loan_status
for each row 
begin
...
create table cibil_table (loan_id varchar(50),loan_amount varchar(50),cibil_score int,cibil_score_status varchar(50));

delimiter //
create trigger cibil_verify after insert on loan_status
for each row
begin 
if new.`Cibil score`> 900 then insert into cibil_table(loan_id,loan_amount,cibil_score,cibil_score_status)
values (new.loan_id,new.loanamount,new.`cibil score`,'High_Cibil_Score');
elseif  new.`Cibil score` > 750 then insert into cibil_table(loan_id,loan_amount,cibil_score,cibil_score_status)
values (new.loan_id,new.loanamount,new.`cibil score`,'NO_Penalty');
elseif  new.`Cibil score` > 0 then insert into cibil_table(loan_id,loan_amount,cibil_score,cibil_score_status)
values (new.loan_id,new.loanamount,new.`cibil score`,'Penalty customers');
elseif  new.`Cibil score` <= 0 then insert into cibil_table(loan_id,loan_amount,cibil_score,cibil_score_status)
values (new.loan_id,new.loanamount,new.`cibil score`,'Reject Customers (Loan cannot apply)');
end if;
end //
delimiter ;
...
delete from cibil_table where loan_amount  = 'Loan Still Processing' or cibil_score_status ='Reject Customers (Loan cannot apply)';

 create table loan_cibilscore_status_details select c.*,s.loan_amount,s.cibil_score,s.cibil_score_status from customer_info c  join cibil_table s on c.loan_id=s.loan_id ; 
 
alter table cust_loan_details modify loan_amount int;

select *from loan_cibilscore_status_details;

 create table customer_interest_analysis select *,((interest_percentage/100) * loan_amount) as mon_interest_amt,
 ((interest_percentage/100) * loan_amount)*12 as ann_interest_amt,
 concat(interest_percentage,'%') as percentage_interest from loan_cibilscore_status_details;
 
 alter table customer_interest_analysis  drop column interest_percentage;
...
create table cust_full_loan_det select l.loan_id,l.`customer id`,l.applicantincome,l.coapplicantincome,l.property_area,l.loan_status,l.grade,l.loan_amount,l.cibil_score,
l.cibil_score_status,l.mon_interest_amt,l.ann_interest_amt,l.percentage_interest,c.customer_name,c.gender,c.age,c.married,c.education,
c.self_employed,c.region_id,s.postal_code,s.segment,s.state,r.region
from customer_interest_analysis l join customer_det c on l.`customer id`=c.`customer id` join 
country_state s on l.`customer id` =s.customer_id join region_info r on c.region_id=r.region_id;

select*from cust_full_loan_det;
...
select * from cust_full_loan_det where cibil_score_status='High_Cibil_Score';
 select * from cust_full_loan_det where segment in ('home office','corporate');

delimiter //
create procedure project_data1()
begin

select * from cust_full_loan_det;

 select c.`load id`,c.customer_id,c.customer_name,c.region_id,c.postal_code,c.segment,c.state,r.region from country_state c
 right join region_info r on c.region_id=r.region_id where `load id` is null;
